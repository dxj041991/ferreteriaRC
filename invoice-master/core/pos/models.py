...
class Purchase(models.Model):
    ...
    @property
    def subtotal(self):
        return float(self.subtotal_without_tax) + float(self.subtotal_with_tax)
    ...
    def calculate_invoice(self):
        ...
        self.total_tax = round(float(self.purchasedetail_set.filter(product__has_tax=True).aggregate(result=Coalesce(Sum('total_tax'), 0.00, output_field=FloatField()))['result']), 2)
    ...
    def as_dict(self):
        item = model_to_dict(self, exclude=['subtotal_without_tax', 'subtotal_with_tax'])
